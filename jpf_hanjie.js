...
*/
// Run the init() function when the page loads. Gives the browser a heads up that it will "onload" the "init" fucntion. puzzleCells is established & defined as a variable for the entire browser.
window.onload = init;
...
function init(){
   ...
   //insert HTML code for the puzzle
   ...
   // When mouse click is released the drag no longer fills any cells
   document.addEventListener("mouseup", endBackground);

   // add an event Listener to the Show Solution Button
   document.getElementById("solve").addEventListener("click", function(){
      // Remove the inline color background color style from each cell
...
function swapPuzzle(e){
   ...
   //Variable that references which puzzle buttown was clicked based on it's ID 
...
function setupPuzzle() {
   ...
   //Loop that will set the initial color
   for(let i = 0; i < puzzleCells.length; i++){
      ...
      // Use a pencil image as the cursor for all the puzzle cells
      ...
      document.getElementById("peek").addEventListener("click", function(){
         ...
         }, 500); // end of the timeout anonymous funtion
      });

      // Check puzzle solution when the user lets go of the mouse button
      document.getElementById("hanjieGrid").addEventListener("mouseup", function(){
         let solved = true;

         // loop through every puzzle cell to look at the color to determine if the color is correct or not.
         for (let i = 0; i < puzzleCells.length; i++) {
...
// definition of setBackground() function
function setBackground(e){
   ...
   } // Here is the end curly brace for the if/else statement for setBackground.0
   ...
   //prevent the default action of selecting table next
   e.preventDefault();
   
} // setBackground function closed 
...
//Definition of endBackground() function
